        ...
        <SheetContent side="left" className="pl-1 pr-0">
          ...
          <ScrollArea className="my-4 h-[calc(100vh-8rem)] pb-10 pl-6">
            <div className="pl-1 pr-7">
              <Accordion type="multiple" defaultValue={items.map((item) => item.title)} className="w-full">
                    ...
                    <AccordionContent>
                      <div className="flex flex-col space-y-2">
                        {item.items?.map((subItem, index) =>
                          subItem.href ? (
                            <MobileLink key={index} href={subItem.href} segment={String(segment)} setIsOpen={setIsOpen}>
                              {subItem.title}
                            </MobileLink>
                          ) : (
